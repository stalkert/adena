...
function get_search_form( $echo = true ) {
	...
	if ( '' != $search_form_template ) {
		...
		$form = ob_get_clean();
	} else {//<span class="screen-reader-text">' . _x( 'Search for:', 'label' ) . '</span>
		if ( 'html5' == $format ) {
			$form = '<form role="search" method="get" class="search-form" action="' . esc_url( home_url( '/' ) ) . '">
				<label>
					
					<input type="search" class="search-field" placeholder="Поиск товаров" value="' . get_search_query() . '" name="s" title="' . esc_attr_x( 'Search for:', 'label' ) . '" />
				</label>
			...
			$form = '<form role="search" method="get" id="searchform" class="searchform" action="' . esc_url( home_url( '/' ) ) . '">
				<div>
					...
					<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="Поиск товаров" />
